        ...
        for message in messages:
            if message["role"] == "system":
                converted_messages.append(
                    {"role": "user", "parts": [f"System: {message['content']}"]}
                )
...
def create_permutation_instruction(
    item=None, rank_start=0, rank_end=100, model_name="gpt-3.5-turbo"
):
    query = item["query"]
...
def permutation_pipeline(
    item=None, rank_start=0, rank_end=100, model_name="gpt-3.5-turbo", api_key=None
):
...
def sliding_windows(
    ...
    while start_pos >= rank_start:
        ...
        item = permutation_pipeline(
            item, start_pos, end_pos, model_name=model_name, api_key=api_key
        )
...
def main():
    ...
    for item in tqdm(rank_results):
        new_item = permutation_pipeline(
            item, rank_start=0, rank_end=20, model_name="gpt-3.5turbo", api_key=api_key
        )
